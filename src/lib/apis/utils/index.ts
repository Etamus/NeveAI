...
export const getGravatarUrl = async (token: string, email: string) => {
	...
	const res = await fetch(`${NEVEAI_API_BASE_URL}/utils/gravatar?email=${email}`, {
		method: 'GET',
		headers: {
...
export const executeCode = async (token: string, code: string) => {
	...
	const res = await fetch(`${NEVEAI_API_BASE_URL}/utils/code/execute`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const formatPythonCode = async (token: string, code: string) => {
	...
	const res = await fetch(`${NEVEAI_API_BASE_URL}/utils/code/format`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const downloadChatAsPDF = async (token: string, title: string, messages: object[]) => {
	...
	const blob = await fetch(`${NEVEAI_API_BASE_URL}/utils/pdf`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const getHTMLFromMarkdown = async (token: string, md: string) => {
	...
	const res = await fetch(`${NEVEAI_API_BASE_URL}/utils/markdown`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const downloadDatabase = async (token: string) => {
	...
	const res = await fetch(`${NEVEAI_API_BASE_URL}/utils/db/download`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
